...
public class ArrowStoreNewestOld implements Store {
	...
	public ArrowStoreNewestOld(final ColumnSchema[] schemas) throws IOException {
		...
		for (int i = 0; i < schemas.length; i++) {
			...
			switch (type) {
			case BOOLEAN: {
				final SingleColumnPartitionStore<BitVector> vectorStore = new DefaultPartitionedColumn<BitVector>(m_baseDirectory, i,
			...
			case DOUBLE: {
				final SingleColumnPartitionStore<Float8Vector> vectorStore = new DefaultPartitionedColumn<Float8Vector>(m_baseDirectory, i,
						/* TODO */ null, /* TODO */ m_allocator) {
			...
			case STRING: {
				final SingleColumnPartitionStore<VarCharVector> vectorStore = new DefaultPartitionedColumn<VarCharVector>(m_baseDirectory, i,
						/* TODO */ null, /* TODO */ m_allocator) {
