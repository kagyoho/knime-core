...
import org.knime.core.data.store.VecFactory;
import org.knime.core.data.store.arrow.table.value.ArrowStringVecReadAccess;
	...
	public VecAccessible create(final ColumnType type) {
		switch (type) {
			case BOOLEAN: {
				...
				return new ArrowVecAccessible<>(vector, ArrowWritableBooleanValueAccess::new, ArrowBooleanVecReadAccess::new);
			}
			case STRING: {
				...
				return new ArrowVecAccessible<>(vector, ArrowWritableStringValueAccess::new, ArrowStringVecReadAccess::new);
			}
			case DOUBLE: {
